...
- Q: How is user identity and session state persisted across runs? → A: Single user per installation — profile and progress stored locally (e.g., `~/.milai/`); no user selection prompt required.
...
The user follows their active curriculum lesson by lesson. Within a lesson, they read theory explanations, complete exercises, and receive immediate feedback. At any point, they may deviate — asking a free-form question, requesting a conversational practice session, or asking the AI to explain something differently. They can also request dynamic changes (skip a lesson, add a topic, change difficulty).
...
- **User Profile**: Represents the single learner on this installation — target language, native language, learning goal, fluency level, time preference; persisted to local storage.
...
- The system is single-user per installation; no login or account selection is required. Profile and progress are stored in the user's local home directory.
